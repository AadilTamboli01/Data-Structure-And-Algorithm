...
public class DijkstrasAlgo {
    ...
    public static void dijkras(ArrayList<Edge> graph[], int src) {
        ...
        // add sord in quueue
        q.add(new Pair(src, 0)); // src to src = 0

        // bfs
        while (!q.isEmpty()) {
            Pair curr = q.remove();
            // if distance not calculate
            if (!vis[curr.n]) {
                vis[curr.n] = true; // vist curr
                // visit neighbors
                for (int i = 0; i < graph[curr.n].size(); i++) {
                    Edge e = graph[curr.n].get(i);
                    int u = e.src;
                    int v = e.dest;
                    int wt = e.wt; // wt of u to v
 

                    // calculation for shortst distance
                    if (dist[u] + wt < dist[v]) {
                        dist[v] = dist[u] + wt;
                        // add in the queue
                        System.out.println("add in queuue = "+(int)v+" ,"+(int)dist[v]);
